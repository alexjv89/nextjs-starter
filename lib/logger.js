...
module.exports=function(req,res){
	...
	if(process.env.DB_SERVERLOG){
		const sequelize = new Sequelize(process.env.DB_SERVERLOG,{
		    dialect: 'postgres',
		    dialectOptions: {
		        ssl: {
	...
	var patt = new RegExp("^\/(_next\/static|static|_next\/image|dependencies|js|semantic|styles|favicon|health|css|min|fonts|image)");
	if (!patt.test(req.url)) {
	    ...
	    console.log('::ServerLog::',JSON.stringify(log));
	    if(ServerLog)
